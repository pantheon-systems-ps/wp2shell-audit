...
- **Database** (requires `--wp`/`--site`): posts with an invalid `post_status` (the whitelist includes WooCommerce's standard order statuses, since those can cover hundreds of thousands of legitimate rows on an active store; a `post_status` breakdown block, cross-referenced against both active plugins *and* installed themes, is printed for anything else non-standard, for a judgment call rather than an automatic flag), forged `customize_changeset` rows (any status), forged `nav_menu_item` rows, `postmeta` rows referencing `example.invalid`, orphaned `usermeta` rows, `<prefix>_<hex>`-style throwaway-admin usernames, and a full list of administrator-role accounts by registration date (for prioritizing the anomaly review).

Five of these six checks are unambiguous on their own and drive a **CONFIRMED COMPROMISE** verdict when non-zero. Invalid `post_status` is the exception — a plugin's or theme's own custom status can produce the same signal with no compromise at all (confirmed both with WooCommerce order statuses and a theme-registered custom post type), so a nonzero count there alone yields **REQUIRES REVIEW** instead, until the breakdown/cross-reference confirms it isn't an installed plugin's or theme's own post type.
